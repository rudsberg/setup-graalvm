...
describe('sbom feature', () => {
  ...
  describe('setup', () => {
    ...
    it('should throw an error when the java version is lower than 24', () => {
      expect(() => setUpSBOMSupport('23.0.0', distribution)).toThrow()
    })
  ...
  describe('process', () => {
    ...
    it('should process SBOM and display components', async () => {
      ...
      expect(spyInfo).toHaveBeenCalledWith(
        'Found SBOM file: ' + join(workspace, 'test.sbom.json')
      )
