...
export function setUpSBOMSupport(
  ...
  if (major(javaVersion) < MIN_JAVA_VERSION) {
    throw new Error(
...
export async function processSBOM(): Promise<void> {
  ...
  try {
    ...
    throw new Error(
      `Failed to process SBOM file: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
interface DependencySnapshot {
  ...
  manifests: Record<
    string,
    {
      ...
      // Not including the 'file' property since we cannot specify any reasonable value for "source_location"
